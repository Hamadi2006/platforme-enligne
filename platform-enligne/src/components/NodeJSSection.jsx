...
export default function NodeJSSection() {
  const NodeJSLessons = useSelector((state) => state.NodeJSState);
  const [selectedLesson, setSelectedLesson] = useState(null);

  const trackData = NodeJSLessons && NodeJSLessons[0];
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <header className="mb-10">
          ...
          <h1 className="text-4xl font-bold text-white mb-3">
            {trackData?.track || 'Node.js Fundamentals'}
          </h1>
          <p className="text-lg text-slate-400 max-w-2xl leading-relaxed">
            {trackData?.description || 'Run JavaScript on the server with Node.js.'}
          </p>
        </header>
