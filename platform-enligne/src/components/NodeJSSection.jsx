function NodeJSSection() {
  return (
    <div className="max-w-6xl mx-auto">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500 to-emerald-500 p-8 md:p-12 mb-8 shadow-2xl">
        <div className="relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-3">Node.js</h1>
          <p className="text-lg md:text-xl text-white/90 max-w-2xl">JavaScript runtime for backend</p>
        </div>
      ...
      <div className="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 lg:p-10 mb-8 border border-slate-800/50 shadow-xl">
        <h2 className="text-2xl md:text-3xl font-bold text-white mb-4 flex items-center gap-3">
          <span className="w-1.5 h-8 bg-gradient-to-b from-green-500 to-emerald-500 rounded-full"></span>
          Overview
        </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {['NPM & Modules', 'Express.js', 'Async Programming', 'REST APIs'].map((topic, index) => (
          <div key={index} className="group bg-slate-900/50 backdrop-blur-sm rounded-xl p-6 border border-slate-800/50 hover:border-slate-700 transition-all duration-300 hover:transform hover:scale-105 cursor-pointer shadow-lg">
            ...
            <p className="text-slate-400 text-sm">Build scalable backend services</p>
          </div>
        ))}
      </div>
    </div>
