...
export default function LaravelSection() {
  const LaravelLessons = useSelector((state) => state.LaravelState);
  const [selectedLesson, setSelectedLesson] = useState(null);

  const trackData = LaravelLessons && LaravelLessons[0];
            ...
            <section className="mb-10">
              ...
              <div className="bg-gradient-to-br from-slate-900 to-slate-900/80 rounded-xl border border-slate-800 overflow-hidden">
                <div className="bg-slate-800/50 px-5 py-3 border-b border-slate-700/50 flex items-center justify-between">
                  <span className="text-xs text-slate-400 font-medium">Laravel</span>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <header className="mb-10">
          ...
          <h1 className="text-4xl font-bold text-white mb-3">
            {trackData?.track || 'Laravel Essentials'}
          </h1>
          <p className="text-lg text-slate-400 max-w-2xl leading-relaxed">
            {trackData?.description || 'Build modern PHP apps with Laravel.'}
          </p>
        </header>
