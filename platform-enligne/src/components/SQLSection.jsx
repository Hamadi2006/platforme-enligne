function SQLSection() {
  return (
    <div className="max-w-6xl mx-auto">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 p-8 md:p-12 mb-8 shadow-2xl">
        <div className="relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-3">SQL</h1>
          <p className="text-lg md:text-xl text-white/90 max-w-2xl">Database query language</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {['SELECT Queries', 'JOIN Operations', 'Data Manipulation', 'Database Design'].map((topic, index) => (
          <div key={index} className="group bg-slate-900/50 backdrop-blur-sm rounded-xl p-6 border border-slate-800/50 hover:border-slate-700 transition-all duration-300 hover:transform hover:scale-105 cursor-pointer shadow-lg">
            ...
            <p className="text-slate-400 text-sm">Master database querying</p>
          </div>
        ))}
      </div>
    </div>
