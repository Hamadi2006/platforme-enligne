...
export default function SQLSection() {
  const SQLLessons = useSelector((state) => state.SQLState);
  const [selectedLesson, setSelectedLesson] = useState(null);

  const trackData = SQLLessons && SQLLessons[0];
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <header className="mb-10">
          ...
          <h1 className="text-4xl font-bold text-white mb-3">
            {trackData?.track || 'SQL Fundamentals'}
          </h1>
          <p className="text-lg text-slate-400 max-w-2xl leading-relaxed">
            {trackData?.description || 'Query and manage data in relational databases.'}
          </p>
        </header>
